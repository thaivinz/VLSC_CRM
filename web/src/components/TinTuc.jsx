      ...
      <main className="ng-tns-c75-0">
        <section className="pt-[30px] pb-[70px] bg-[#eff6ff] min-h-[800px]">
          <div className="container !max-w-[1200px] mx-auto px-[15px]">
            <div className="flex md:flex-row flex-col justify-between items-center pb-[20px] lg:pb-[60px] w-full">
              <ul className="flex mt-[12px] flex-wrap w-full md:w-auto">
                ...
                <li>
                  <span className="text-[#0063E2] font-semibold">Tin tức</span>
                </li>
              </ul>
              <div className="relative text-[#1B2C43] md:w-auto w-full h-[42px] cursor-pointer z-10 mt-[15px] md:mt-0">
                <span className="absolute left-[12px] top-[11px] z-10 duration-300 pointer-events-none">
                  <svg role="img" aria-hidden="true" className="svg-icon-search" style={{ fontSize: '20px' }} width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16">
                    <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"></path>
                  </svg>
                </span>
                <input type="text" className="h-[42px] pl-[42px] rounded-[30px] lg:w-[350px] md:w-[263px] w-full bg-[#F0F3F5] outline-0 transition-all duration-300 p-[8px_10px_8px_38px] text-[#1B2C43]" placeholder="Tìm kiếm tin tức" />
              </div>
            </div>

            <h3 className="text-[34px] text-[#1B2C43] font-semibold text-center mb-[50px]">Tin tức mới nhất</h3>

            <div className="max-w-[900px] mx-auto">
              {news.map((item) => (
                <div key={item.id} className="mb-[24px]">
                  <div className="flex p-[20px] bg-white rounded-[26px] gap-[24px] shadow-[0px_4px_20px_rgba(0,52,99,0.1)] md:!h-[200px] h-auto xs:h-[200px]">
                    <a href={`/tin-tuc/${item.id}`} className="shrink-0 rounded-[14px] overflow-hidden w-[70px] xs:w-[127px] md:!w-[170px] lg:!w-[215px] block">
                      <img className="w-full h-full max-w-full object-cover transition-all duration-300 hover:scale-105 cursor-pointer" alt={item.title} src={item.image} />
                    </a>
                    <div className="flex-1 space-y-3 overflow-hidden">
                      <h3 className="line-clamp-2 md:line-clamp-3">
                        <a href={`/tin-tuc/${item.id}`} className="font-bold text-[#23273C] text-[16px] md:text-[20px] cursor-pointer hover:text-[#D72134]">{item.title}</a>
                      </h3>
                      <p className="flex items-center gap-[10px] text-[12px] md:text-[14px] text-[#D72134] font-medium">
                        <svg role="img" aria-hidden="true" className="svg-icon-calendar-web" style={{ fontSize: '1rem' }} width="1em" height="1em" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
